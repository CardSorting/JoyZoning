...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Workspace containment check (strict read protection) ──────
        from agent.file_safety import get_safe_write_root, log_blocked_attempt
        safe_root = get_safe_write_root()
        if safe_root:
            resolved_str = str(_resolved)
            is_inside = (resolved_str == safe_root or resolved_str.startswith(safe_root + os.sep))
            if not is_inside:
                log_blocked_attempt(resolved_str)
                return json.dumps({
                    "error": f"Containment violation: Read target '{path}' resolves to '{resolved_str}' which is outside the allowed workspace root '{safe_root}'."
                })
