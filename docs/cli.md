...
- **Workspace inspection** — `/workspace changed|tree|diff` uses the active task’s **canonical workspace** (card branch when leased) when `/use <task>` is set; otherwise `--root`
