...
Parallel live mirrors: `GET /api/sessions/{sessionId}/parallel-workers` (read model from `.joyzoning/live/index.json`, registry, and disk meta). Merge/reconciliation queue: `GET /api/sessions/{sessionId}/merge-queue` (buckets: ready to merge, conflicts, completed, revoked/abandoned; includes `mergeState`, `mergeReadiness`, `mergeConflict`, `decisionSummary`, `approveGuardrails`, `revokeGuardrails`). Operator preflight: `GET /api/sessions/{sessionId}/workers/{executionSessionId}/decision-preflight?action=approve|revoke|inspect`. Human actions: `POST /api/tasks/{taskId}/lease/merge`, `POST /api/tasks/{taskId}/lease/revoke`. Open mirror folder (macOS/Linux): `POST /api/sessions/open-path` with `{ "path": "..." }`. **Convergence model (worktree → main workspace, what approve does):** [worker-convergence.md](worker-convergence.md).
